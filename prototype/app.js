import { kpis, liteCatalog } from './data.js?v=17';
import { protonSuiteById, protonModuleByRoute } from './proton-modules.js?v=17';
import {
  ...
  sortLiteDictionary, updateLiteAlert, resetTenantUserPassword, reviewLiteDocument, createLitePrintJob, completeLitePrintJob,
  refreshOperations, createExportJob, simulateWaveRules, autoAssignTasks, reconcileInventory,
  scanAsn, createCountPlan, scanRma, releaseReplenishment, simulateRuleSample,
  syncMasterData, saveLocation, resolveApproval, runProtonModule, syncProtonSuite,
} from './state.js?v=17';
import { createUiState, paginateRows, selectTab, selectTwinLayer, visibleTwinParts, searchWorkspace } from './ui-state.js?v=17';
...
function businessModulePanel(suiteId, pageKey, title = '业务能力工作台') {
  const suite = protonSuiteById(suiteId);
  if (!suite) return '';
  const meta = paginateRows(suite.modules, ui.pages[pageKey] || 1, 6);
  ui.pages[pageKey] = meta.page;
  const runs = state.proton.moduleRuns.filter((item) => item.suiteId === suite.id);
  const statusFor = (route) => state.proton.moduleStatus[route] || '待执行';
  const rows = meta.items.map((module) => `<tr>
    <td data-label="功能"><b>${module.name}</b><br><span class="mono muted">${module.route}</span></td>
    <td data-label="对象" class="mono">${module.object}</td>
    <td data-label="动作" class="wrap-cell">${module.actions.join(' / ')}</td>
    <td data-label="状态">${badge(statusFor(module.route), statusFor(module.route) === '已完成' ? 'success' : statusFor(module.route) === '已同步' ? 'info' : 'warning')}</td>
    <td data-label="操作"><button class="link-button" data-action="proton-open" data-proton-route="${module.route}">详情</button> · <button class="link-button" data-action="proton-run" data-proton-suite="${suite.id}" data-proton-route="${module.route}">${suite.primary}</button></td>
  </tr>`).join('');
  return `<article class="panel table-panel embedded-module"><header class="panel-head"><div class="panel-title"><h2>${title}</h2><p>${suite.desc}，已融入当前业务页面执行</p></div><div class="head-actions"><span class="badge info"><i></i>${suite.modules.length} 项功能</span><button class="btn secondary" data-action="proton-sync" data-proton-suite="${suite.id}">同步配置</button></div></header><div class="table-wrap"><table class="data-table"><thead><tr><th>功能</th><th>业务对象</th><th>关键动作</th><th>状态</th><th>操作</th></tr></thead><tbody>${rows}</tbody></table></div>${paginationControls(pageKey, meta)}${runs.length ? `<div class="module-run-strip">${runs.slice(0, 3).map((item) => `<span>${item.moduleName} · ${item.action}</span>`).join('')}</div>` : ''}</article>`;
}
...
function render() {
  const views = { dashboard: dashboardView, orders: ordersView, waves: wavesView, tasks: tasksView, shipping: shippingView, exceptions: exceptionsView, twin: twinView, inventory: inventoryView, inbound: inboundView, documents: () => liteWmsView({ module: 'documents', title: '业务单据', desc: '入库、出库、移库、盘库单据可审核、驳回、打印，并驱动库存事务' }), transactions: () => liteWmsView({ module: 'transactions', title: '库存事务', desc: '单据审核后生成入账、扣减、移库和盘点调整记录，支持链路追踪' }), alerts: () => liteWmsView({ module: 'warnings', title: '预警设置', desc: '按 SKU 和仓库配置库存上下限，随业务事务自动刷新当前值' }), printJobs: () => liteWmsView({ module: 'printJobs', title: '打印队列', desc: '由单据和模板生成打印任务，支持排队、完成和审计追踪' }), reports: reportsView, finance: financeView, replenishment: replenishmentView, count: countView, returns: returnsView, master: masterView, rules: rulesView, integrations: integrationsView, printing: printingView, admin: adminView, pda: pdaView, lite: liteWmsView };
...
function ordersView() {
  ...
  return `<section class="page">${pageHead('OUTBOUND', '出库订单池', '统一校验、分配并组织来自多渠道的履约需求', `<button class="btn secondary" data-action="export">导出</button><button class="btn primary" data-action="allocate">校验并分配 <span id="selected-count">${state.selectedOrders.length?`(${state.selectedOrders.length})`:''}</span></button>`)}
    ...
    ${businessModulePanel('protonOutbound','outboundOps','出库业务功能')}
  </section>`;
}
...
function inventoryView() {
  ...
  return `<section class="page">${pageHead('INVENTORY LEDGER', '库存中心', '以库位、批次和库存状态为主线，统一查看可用、占用与冻结余额', '<button class="btn secondary" data-action="inventory-reconcile">库存对账</button><button class="btn primary" data-action="open-freeze">冻结库存</button>')}
    ...
    ${businessModulePanel('protonInventory','inventoryOps','库存业务功能')}
  </section>`;
}
...
function inboundView() {
  ...
  return `<section class="page">${pageHead('INBOUND RECEIVING', '入库管理', '从 ASN 预约、收货差异到质检与上架任务的可追溯闭环', '<button class="btn secondary" data-action="scan-asn">模拟扫描 ASN</button>')}
    ...
    <article class="panel table-panel" style="margin-top:16px"><header class="panel-head"><div class="panel-title"><h2>入库单据工作台</h2><p>覆盖入库单、退货入库单、收货记录编辑与审核</p></div><button class="btn secondary" data-action="new-document">新增单据</button></header>${liteTable(['单据号','单据名称','类型','往来单位/库区','数量','金额','状态'], documents, '编辑')}</article>
    ${businessModulePanel('protonInbound','inboundOps','入库业务功能')}
  </section>`;
}
...
function reportsView() {
  ...
  return `<section class="page">${pageHead('OPERATIONS ANALYTICS', '分析报表', '统一观察入库、库存、出库和人员作业效率，定位趋势与瓶颈', `<select class="select" id="report-range" aria-label="报表时间范围"><option ${ui.tabs.reports==='today'?'selected':''}>今日</option><option ${ui.tabs.reports==='week'?'selected':''}>本周</option><option ${ui.tabs.reports==='month'?'selected':''}>本月</option></select><button class="btn secondary" data-action="report-export">导出报表</button>`)}
    ...
    ${businessModulePanel('protonReports','reportOps','报表业务功能')}
  </section>`;
}

function financeView() {
  const billRows = [
    ['BILL260710-009', '星云智能科技', '仓储+操作+物流', '¥42,680', '待确认'],
    ['BILL260710-008', '海川贸易', '仓租+增值服务', '¥18,240', '已确认'],
    ['ABILL260710-003', '星云智能科技', '称重差异调整', '-¥260', '审批中'],
  ];
  return `<section class="page">${pageHead('BILLING', '计费财务', '围绕报价、充值、费用账单和调整账单形成可追溯的仓储计费闭环', '<button class="btn secondary" data-action="report-export">导出财务清单</button><button class="btn primary" data-action="proton-run-first" data-proton-suite="protonFinance">生成计费结果</button>')}
    <div class="kpi-grid"><article class="kpi-card"><div class="kpi-label">本月应收</div><div class="kpi-value">¥168K</div><div class="kpi-meta"><b>7 个客户</b></div></article><article class="kpi-card success"><div class="kpi-label">已确认账单</div><div class="kpi-value">12</div><div class="kpi-meta"><b>92.4%</b></div></article><article class="kpi-card warning"><div class="kpi-label">待确认</div><div class="kpi-value">3</div><div class="kpi-meta"><b>含 1 个调整单</b></div></article><article class="kpi-card"><div class="kpi-label">报价版本</div><div class="kpi-value">18</div><div class="kpi-meta"><b>仓租/操作/服务</b></div></article></div>
    <div class="split-layout"><article class="panel table-panel"><header class="panel-head"><div class="panel-title"><h2>账单处理</h2><p>报价生效后按作业事务、仓租、物流和增值服务生成账单</p></div>${badge('财务可确认','info')}</header><div class="table-wrap"><table class="data-table"><thead><tr><th>单号</th><th>客户</th><th>费用类型</th><th>金额</th><th>状态</th><th>操作</th></tr></thead><tbody>${billRows.map((row)=>`<tr>${row.map((cell,index)=>`<td data-label="${['单号','客户','费用类型','金额','状态'][index]}">${index===4?badge(cell,cell==='已确认'?'success':cell==='审批中'?'warning':'info'):cell}</td>`).join('')}<td><button class="link-button" data-action="proton-run" data-proton-suite="protonFinance" data-proton-route="/financeBill/list">确认账单</button></td></tr>`).join('')}</tbody></table></div></article><aside class="panel summary-card"><h2>计费规则</h2><div class="rule-list"><div class="rule-item"><i>✓</i>报价版本生效后才参与计费</div><div class="rule-item"><i>✓</i>费用账单确认后写入财务审计</div><div class="rule-item"><i>!</i>调整账单必须经过审批</div></div></aside></div>
    ${businessModulePanel('protonFinance','financeOps','财务业务功能')}
  </section>`;
}
...
function masterView() {
  const catalogs=state.lite.catalogs;
  const datasets={
    warehouses:{headers:['仓库编码','仓库名称','排序','创建人','创建时间','状态'],rows:catalogs.warehouses.map(item=>[item.code,item.name,item.orderNum,item.createBy,item.createTime,item.status])},
    items:{headers:['SKU','商品名称','分类','品牌','单位','库存属性','状态'],rows:catalogs.items.map(item=>[item.sku,item.name,item.category,item.brand,item.unit,item.attrs,item.status])},
    dictionaries:{headers:['字典类型','编码','名称','排序','状态'],rows:catalogs.itemDictionaries.map(item=>[item.type,item.code,item.name,item.orderNum,item.status])},
    merchants:{headers:['往来单位编码','名称','类型','联系方式','结算信息','状态'],rows:catalogs.merchants.map(item=>[item.code,item.name,item.type,item.contact,item.settlement,item.status])},
    people:{headers:['账号','姓名','角色','仓库权限','最近登录','状态'],rows:catalogs.people.map(item=>[item.account,item.name,item.role,item.warehouseType,item.loginDate,item.status])},
    operationTypes:{headers:['操作类型编码','名称','方向','库存影响','审核节点','状态'],rows:catalogs.operationTypes.map(item=>[item.code,item.name,item.direction,item.affects,item.audit,item.status])},
  ...
  return `<section class="page">${pageHead('MASTER DATA', '主数据中心', '融合线上系统主数据能力，覆盖仓库、商品、品牌分类单位属性、人员、往来单位和操作类型配置','<button class="btn secondary" data-action="sync-master">同步主数据</button><button class="btn primary" data-action="new-master">新增主数据</button>')}<div class="toolbar"><div class="segmented">${tabButton('master','warehouses','仓库管理')}${tabButton('master','items','商品管理')}${tabButton('master','dictionaries','品牌/分类/单位/属性')}${tabButton('master','merchants','往来单位')}${tabButton('master','people','人员信息')}${tabButton('master','operationTypes','操作类型')}</div><label class="search"><span>⌕</span><input class="list-search" type="search" placeholder="搜索当前列表"></label></div><article class="panel table-panel"><div class="table-wrap"><table class="data-table"><thead><tr>${dataset.headers.map(h=>`<th>${h}</th>`).join('')}<th>操作</th></tr></thead><tbody>${meta.items.map(r=>`<tr>${r.map((c,i)=>`<td data-label="${dataset.headers[i]}">${i===dataset.headers.length-1?badge(c,c==='启用'?'success':c==='锁定'?'danger':'warning'):c}</td>`).join('')}<td><button class="link-button" data-action="lite-detail">编辑 →</button></td></tr>`).join('')}</tbody></table></div>${paginationControls('master',meta)}</article>${businessModulePanel('protonMaster','masterOps','主数据业务功能')}</section>`;
}

function liteWmsView(options = {}) {
  const tabs = [
    ['warehouses','仓库管理'], ['items','商品/SKU'], ['dictionaries','品牌分类单位属性'],
    ['merchants','往来单位'], ['operationTypes','操作类型'], ['people','人员信息'],
    ['warnings','预警设置'], ['documents','出入移盘单据'], ['transactions','库存事务'], ['printing','打印模板'], ['printJobs','打印队列'], ['system','系统受限项'],
  ];
  const active = options.module || ui.tabs.lite || 'warehouses';
  ...
  const headers = {
    ...
    transactions:['事务号','来源单据','类型','影响对象','数量','状态'],
    printing:['模板编码','模板名称','业务场景','匹配规则','状态'],
    printJobs:['任务号','模板','来源单据','打印机','份数','状态'],
  ...
  const rows = {
    ...
    transactions:state.lite.inventoryTransactions.map(i=>[i.id,i.documentId,i.documentType,i.item,i.quantity,i.status,i.id]),
    ...
    printJobs:state.lite.printJobs.map(i=>[i.id,i.templateName,i.sourceId,i.printer,i.copies,i.status,i.id]),
  ...
  const actionCell = (row) => {
    ...
    if (active === 'transactions') return '<td><button class="link-button" data-action="lite-transaction-detail">查看链路</button></td>';
    ...
    if (active === 'printJobs') return `<td><button class="link-button" data-action="lite-print-complete" data-lite-id="${id}">完成打印</button></td>`;
  ...
  const tableRows = currentRows.length ? currentRows.map(row=>`<tr>${row.slice(0,-1).map((cell,index)=>`<td data-label="${currentHeaders[index]}">${index===currentHeaders.length-1?badge(cell,cell==='启用'||cell==='正常'||cell==='已审核'||cell==='已入账'||cell==='已完成'?'success':cell==='预警'||cell==='异常'||cell==='停用'?'danger':cell==='受限'||cell==='待审核'||cell==='排队中'?'warning':'muted'):cell}</td>`).join('')}${actionCell(row)}</tr>`).join('') : `<tr><td colspan="${currentHeaders.length + 1}"><div class="empty-state" style="min-height:150px"><div><h2>暂无${tabs.find(([id])=>id===active)?.[1] || '记录'}</h2><p>完成单据审核或生成打印任务后，这里会自动出现业务结果。</p></div></div></td></tr>`;
  ...
  const tabToolbar = options.module ? '' : `<div class="segmented">${tabs.map(([id,label])=>tabButton('lite',id,label)).join('')}</div>`;
  const newActionLabel = active === 'documents' ? '新增单据' : active === 'warnings' ? '新增预警' : active === 'printJobs' ? '创建打印任务' : '新增当前模块';
  const kpiByModule = (() => {
    if (active === 'documents') return [
      ['待审核单据', data.documentTemplates.filter(item=>item.status==='待审核').length, '需业务审核', 'warning'],
      ['已审核单据', data.documentTemplates.filter(item=>item.status==='已审核').length, '可入账追踪', 'success'],
      ['已驳回单据', data.documentTemplates.filter(item=>item.status==='已驳回').length, '待经办修正', 'danger'],
      ['关联打印任务', state.lite.printJobs.filter(item=>String(item.sourceId).includes('260703')).length, '单据触发', ''],
    ];
    if (active === 'transactions') return [
      ['库存事务', state.lite.inventoryTransactions.length, '已落账', 'success'],
      ['入库影响', state.lite.inventoryTransactions.filter(item=>item.quantity > 0).reduce((sum,item)=>sum+Number(item.quantity),0), '增加库存', ''],
      ['出库影响', Math.abs(state.lite.inventoryTransactions.filter(item=>item.quantity < 0).reduce((sum,item)=>sum+Number(item.quantity),0)), '扣减库存', 'warning'],
      ['待追踪链路', state.lite.inventoryTransactions.filter(item=>item.status==='已入账').length, '可查看', ''],
    ];
    if (active === 'warnings') return [
      ['正常对象', data.inventoryWarnings.filter(item=>item.level==='正常').length, '阈值内', 'success'],
      ['预警对象', data.inventoryWarnings.filter(item=>item.level==='预警').length, '需关注', 'warning'],
      ['异常配置', data.inventoryWarnings.filter(item=>item.level==='异常').length, '需修正', 'danger'],
      ['重算来源', state.lite.inventoryTransactions.length, '库存事务', ''],
    ];
    if (active === 'printJobs') return [
      ['排队任务', state.lite.printJobs.filter(item=>item.status==='排队中').length, '待打印', 'warning'],
      ['完成任务', state.lite.printJobs.filter(item=>item.status==='已完成').length, '已闭环', 'success'],
      ['默认打印机', 'PT-LITE-01', '在线', ''],
      ['可用模板', 4, '业务单据', ''],
    ];
    return [
      ['真实接口模块', 14, '租户侧 API', ''],
      ['库存事务', state.lite.inventoryTransactions.length, '审核后入账', 'success'],
      ['打印队列', state.lite.printJobs.length, `${state.lite.printJobs.filter(item=>item.status==='排队中').length} 排队`, 'warning'],
      ['待审核', state.lite.pendingAudit.filter(item=>item.status==='待审核').length, '主数据发布', ''],
    ];
  })();
  const independentPanel = (() => {
    if (!options.module) return `<div class="mini-grid" style="margin-top:16px"><article class="panel table-panel"><header class="panel-head"><div class="panel-title"><h2>主数据审核</h2><p>仅用于主数据新增、编辑后的发布审批</p></div></header><div class="table-wrap"><table class="data-table"><thead><tr><th>审批单</th><th>模块</th><th>对象</th><th>状态</th><th>操作</th></tr></thead><tbody>${auditRows}</tbody></table></div></article><article class="panel"><header class="panel-head"><div class="panel-title"><h2>主数据变更记录</h2><p>仅记录主数据配置操作</p></div></header><div class="panel-body timeline">${state.lite.auditTrail.filter(item=>!['单据审核通过','单据驳回','生成打印任务','打印完成'].includes(item.action)).slice(-5).reverse().map(item=>`<div class="timeline-item"><b>${item.action}</b><small>${item.time} · ${item.actor} · ${item.target}</small></div>`).join('')}</div></article></div>`;
    if (active === 'documents') return `<div class="mini-grid" style="margin-top:16px"><article class="panel panel-body"><h2 style="font-size:14px;margin-top:0">单据流转状态</h2><div class="rule-list"><div class="rule-item"><i>✓</i>待审核：${data.documentTemplates.filter(item=>item.status==='待审核').length} 单</div><div class="rule-item"><i>✓</i>已审核：${data.documentTemplates.filter(item=>item.status==='已审核').length} 单</div><div class="rule-item"><i>✓</i>已驳回：${data.documentTemplates.filter(item=>item.status==='已驳回').length} 单</div></div></article><article class="panel panel-body"><h2 style="font-size:14px;margin-top:0">后续影响</h2><div class="rule-list"><div class="rule-item"><i>✓</i>审核通过生成库存事务</div><div class="rule-item"><i>✓</i>打印生成打印队列任务</div><div class="rule-item"><i>✓</i>驳回保留原因并等待经办人修正</div></div></article></div>`;
    if (active === 'transactions') return `<article class="panel table-panel" style="margin-top:16px"><header class="panel-head"><div class="panel-title"><h2>事务入账结果</h2><p>库存事务只展示已落账业务，不再重复审批队列</p></div>${badge(`${state.lite.inventoryTransactions.length} 条事务`,'info')}</header><div class="panel-body timeline">${state.lite.inventoryTransactions.length ? state.lite.inventoryTransactions.slice(-5).reverse().map(item=>`<div class="timeline-item"><b>${item.id} · ${item.status}</b><small>${item.documentId} · ${item.documentType} · ${item.quantity}</small></div>`).join('') : '<div class="empty-state" style="min-height:120px"><div><h2>暂无事务链路</h2><p>审核业务单据后会生成库存事务。</p></div></div>'}</div></article>`;
    if (active === 'warnings') return `<article class="panel table-panel" style="margin-top:16px"><header class="panel-head"><div class="panel-title"><h2>预警重算结果</h2><p>阈值变化和单据入账会刷新当前值与预警等级</p></div></header><div class="panel-body"><div class="stat-pair"><div class="stat-box"><small>正常</small><b>${data.inventoryWarnings.filter(item=>item.level==='正常').length}</b></div><div class="stat-box"><small>预警</small><b>${data.inventoryWarnings.filter(item=>item.level==='预警').length}</b></div><div class="stat-box"><small>异常</small><b>${data.inventoryWarnings.filter(item=>item.level==='异常').length}</b></div><div class="stat-box"><small>最近事务</small><b>${state.lite.inventoryTransactions.length}</b></div></div></div></article>`;
    if (active === 'printJobs') return `<article class="panel table-panel" style="margin-top:16px"><header class="panel-head"><div class="panel-title"><h2>打印闭环</h2><p>排队任务完成后状态会从排队中变为已完成</p></div></header><div class="panel-body"><div class="stat-pair"><div class="stat-box"><small>排队中</small><b>${state.lite.printJobs.filter(item=>item.status==='排队中').length}</b></div><div class="stat-box"><small>已完成</small><b>${state.lite.printJobs.filter(item=>item.status==='已完成').length}</b></div><div class="stat-box"><small>打印机</small><b>PT-LITE-01</b></div><div class="stat-box"><small>模板数</small><b>4</b></div></div></div></article>`;
    return '';
  })();
  return `<section class="page">${pageHead('WMS OPERATIONS', options.title || '在线 WMS 管理台', options.desc || '列表、增改删、审核、启停、排序、预警、人员和模板均按业务闭环流转', `<button class="btn secondary" data-action="lite-sync">同步业务数据</button><button class="btn primary" data-action="${active === 'documents' ? 'new-document' : active === 'printJobs' ? 'new-print' : 'lite-new'}" ${writable?'':'disabled'}>${newActionLabel}</button>`)}
    <div class="kpi-grid">${kpiByModule.map(([label,value,meta,tone])=>`<article class="kpi-card ${tone}"><div class="kpi-label">${label}</div><div class="kpi-value">${value}</div><div class="kpi-meta"><b>${meta}</b></div></article>`).join('')}</div>
    <div class="toolbar">${tabToolbar}<label class="search"><span>⌕</span><input class="list-search" type="search" placeholder="搜索当前模块"></label>${active==='dictionaries'?'<button class="btn secondary" data-action="lite-sort">保存排序</button>':''}</div>
...
function liteEndpoint(module) {
  return ({
    ...
    transactions:'审核后生成库存事务，联动预警当前值',
    printing:'hiprint 打印模板、预览、复制、受控重打',
    printJobs:'模板生成打印任务，支持队列完成状态',
...
function adminView() {
  ...
  const pendingApprovals=sec.approvals.filter(item=>item.status==='审批中');
  const processedApprovals=sec.approvals.filter(item=>item.status!=='审批中');
  const urgentApprovals=pendingApprovals.filter(item=>item.risk==='高');
  const approvals=`<div class="kpi-grid"><article class="kpi-card warning"><div class="kpi-label">待我审批</div><div class="kpi-value">${pendingApprovals.length}</div><div class="kpi-meta"><b>${urgentApprovals.length} 项高风险</b></div></article><article class="kpi-card"><div class="kpi-label">即将超时</div><div class="kpi-value">${pendingApprovals.filter(item=>item.sla.includes('38')).length}</div><div class="kpi-meta"><b>${pendingApprovals[0]?.sla || '暂无待处理'}</b></div></article><article class="kpi-card success"><div class="kpi-label">今日已处理</div><div class="kpi-value">${12 + processedApprovals.length}</div><div class="kpi-meta"><b>含本次 ${processedApprovals.length} 项</b></div></article><article class="kpi-card"><div class="kpi-label">执行失败</div><div class="kpi-value">${sec.approvals.filter(item=>item.status==='执行失败').length}</div><div class="kpi-meta"><b>受控事务正常</b></div></article></div><article class="panel table-panel"><div class="table-wrap"><table class="data-table"><thead><tr><th>审批单</th><th>类型</th><th>申请人</th><th>业务对象</th><th>变更摘要</th><th>风险</th><th>当前节点 / SLA</th><th>操作</th></tr></thead><tbody>${sec.approvals.map(a=>`<tr><td class="mono">${a.id}</td><td>${a.type}</td><td>${a.applicant}</td><td>${a.object}</td><td>${a.summary}</td><td>${badge(a.risk==='高'?'高风险':a.risk,a.risk==='高'?'danger':'warning')}</td><td>${a.node}<br><span class="muted">${a.sla}</span><br>${badge(a.status,a.status==='已通过'?'success':a.status==='已驳回'?'danger':'warning')}</td><td>${a.status==='审批中'?`<button class="link-button" data-action="approval-detail" data-approval-id="${a.id}">处理 →</button>`:`<button class="link-button" data-action="approval-result" data-approval-id="${a.id}">查看结果</button>`}</td></tr>`).join('')}</tbody></table></div></article>`;
  ...
  return `<section class="page">${pageHead('SECURITY GOVERNANCE', '安全治理中心', '管理用户、角色、有效权限、审批流程与不可变审计证据','<button class="btn secondary" data-action="security-report">安全态势报告</button>')}${tabs}${content}${businessModulePanel('protonSystem','systemOps','系统安全功能')}</section>`;
}
...
document.addEventListener('click', (event) => {
  ...
  if (action === 'proton-open') {
    const route = event.target.closest('[data-proton-route]').dataset.protonRoute;
    const found = protonModuleByRoute(route);
    const status = state.proton.moduleStatus[route] || '待执行';
    const logs = state.proton.moduleRuns.filter((item) => item.route === route);
    openDrawer('参考系统模块详情', `<div class="detail-section"><div style="display:flex;justify-content:space-between;align-items:center;gap:12px"><span><b>${found.module.name}</b><br><small class="mono">${found.module.route}</small></span>${badge(status, status === '已完成' ? 'success' : 'warning')}</div></div><div class="detail-grid"><div class="detail-cell"><small>所属套件</small><b>${found.suite.title}</b></div><div class="detail-cell"><small>业务对象</small><b>${found.module.object}</b></div><div class="detail-cell"><small>接口路径</small><b class="mono">${found.module.api}</b></div><div class="detail-cell"><small>已执行</small><b>${logs.length} 次</b></div></div><div class="detail-section"><h3>支持动作</h3><div class="rule-list">${found.module.actions.map((item) => `<div class="rule-item"><i>✓</i>${item}</div>`).join('')}</div></div><div class="detail-section"><h3>执行记录</h3><div class="timeline">${logs.length ? logs.map((item) => `<div class="timeline-item"><b>${item.action}</b><small>${item.time} · ${item.result}</small></div>`).join('') : '<div class="timeline-item"><b>等待首次执行</b><small>点击模块动作后生成业务结果</small></div>'}</div></div><div class="action-bar"><button class="btn primary" data-action="proton-run" data-proton-suite="${found.suite.id}" data-proton-route="${found.module.route}">${found.suite.primary}</button></div>`, event.target);
  } else if (action === 'proton-sync') {
    const suiteId = event.target.closest('[data-proton-suite]').dataset.protonSuite;
    const r = syncProtonSuite(state, suiteId, protonSuiteById(suiteId));
    state = r.state; toast(r.message, r.ok, '套件同步完成'); render();
  } else if (action === 'proton-run' || action === 'proton-run-first') {
    const suiteId = event.target.closest('[data-proton-suite]').dataset.protonSuite;
    const suite = protonSuiteById(suiteId);
    const route = event.target.closest('[data-proton-route]')?.dataset.protonRoute || suite.modules[0].route;
    const found = protonModuleByRoute(route);
    const r = runProtonModule(state, { suite: found.suite, module: found.module });
    state = r.state; toast(r.message, r.ok, '业务动作已完成'); render();
  ...
  else if (action === 'approval-detail') {
    const id=event.target.closest('[data-approval-id]')?.dataset.approvalId || 'AP260703-018';
    const approval=state.security.approvals.find(item=>item.id===id);
    openDrawer('高风险审批', `<div class="detail-grid"><div class="detail-cell"><small>申请人</small><b>${approval.applicant}</b></div><div class="detail-cell"><small>变更摘要</small><b>${approval.summary}</b></div><div class="detail-cell"><small>对象</small><b>${approval.object}</b></div><div class="detail-cell"><small>风险等级</small><b>${approval.risk}</b></div></div><div class="timeline"><div class="timeline-item"><b>提交${approval.type}申请</b><small>10:16 · 工作站 WS-08</small></div><div class="timeline-item"><b>系统校验进入审批</b><small>${approval.sla} · ${approval.node}</small></div></div><div class="action-bar"><button class="btn secondary" data-action="reject-approval" data-approval-id="${approval.id}">驳回</button><button class="btn primary" data-action="approve" data-approval-id="${approval.id}">批准并生成事务</button></div>`,event.target);
  }
  else if (action === 'approval-result') {
    const id=event.target.closest('[data-approval-id]')?.dataset.approvalId;
    const approval=state.security.approvals.find(item=>item.id===id);
    openDrawer('审批处理结果', `<div class="detail-grid"><div class="detail-cell"><small>审批单</small><b>${approval.id}</b></div><div class="detail-cell"><small>状态</small><b>${approval.status}</b></div><div class="detail-cell"><small>业务对象</small><b>${approval.object}</b></div><div class="detail-cell"><small>节点</small><b>${approval.node}</b></div></div><div class="weight-result">${approval.summary} · ${approval.sla}</div>`, event.target);
  }
  else if (action === 'locate-alert') { state.twin.selectedZone='picking'; render(); shortPickDrawer(event.target); }
  else if (action === 'refresh') { const r=refreshOperations(state); state=r.state; toast(r.message,r.ok,'同步完成'); render(); }
  else if (action === 'export') { const r=createExportJob(state,{type:'脱敏订单清单',scope:'当前筛选订单'}); state=r.state; toast(r.message,r.ok,'导出任务已创建'); render(); }
  else if (action === 'simulate') { const r=simulateWaveRules(state); state=r.state; toast(r.message,r.ok,'规则试算完成'); render(); }
  else if (action === 'auto-assign') { const r=autoAssignTasks(state); state=r.state; toast(r.message,r.ok,'任务分配完成'); render(); }
  else if (action === 'print') { const r=createLitePrintJob(state,{templateId:'TPL-SHP',sourceId:state.package.id}); state=r.state; toast(r.message,r.ok,'面单任务已创建'); render(); }
  else if (action === 'inventory-reconcile') { const r=reconcileInventory(state); state=r.state; toast(r.message,r.ok,'库存对账完成'); render(); }
  else if (action === 'scan-asn') { const r=scanAsn(state); state=r.state; toast(r.message,r.ok,'扫描成功'); render(); }
  else if (action === 'report-export') { const r=createExportJob(state,{type:'运营日报',scope:ui.tabs.reports}); state=r.state; toast(r.message,r.ok,'报表已生成'); render(); }
  else if (action === 'rules') openDrawer('异常规则配置', '<div class="detail-grid"><div class="detail-cell"><small>短拣阈值</small><b>差异 ≥ 1 件自动阻断</b></div><div class="detail-cell"><small>称重差异</small><b>±0.5kg</b></div><div class="detail-cell"><small>接口重试</small><b>3 次后转人工</b></div><div class="detail-cell"><small>恢复策略</small><b>补拣 / 替代 / 释放</b></div></div><div class="action-bar"><button class="btn secondary" data-action="rule-simulate">规则试算</button><button class="btn primary" data-action="open-rule">创建新版本</button></div>', event.target);
  else if (action === 'dispatch') openDrawer('叉车恢复派单', '<div class="detail-grid"><div class="detail-cell"><small>设备</small><b>FL-03</b></div><div class="detail-cell"><small>最后心跳</small><b>6 分钟前</b></div><div class="detail-cell"><small>影响区域</small><b>存储区 A</b></div><div class="detail-cell"><small>接单人</small><b>设备管理员 · 侯明</b></div></div><div class="timeline"><div class="timeline-item"><b>诊断任务已生成</b><small>优先级：高 · SLA 15 分钟</small></div><div class="timeline-item"><b>关联补货任务暂停</b><small>避免无设备占用通道</small></div></div>', event.target);
  else if (action === 'replenish-simulate') openDrawer('补货需求试算', '<div class="detail-grid"><div class="detail-cell"><small>命中 SKU</small><b>18</b></div><div class="detail-cell"><small>合并重复需求</small><b>6 条</b></div><div class="detail-cell"><small>预计补货量</small><b>2,486 件</b></div><div class="detail-cell"><small>产能占用</small><b>74%</b></div></div><div class="rule-list"><div class="rule-item"><i>✓</i>已排除正在盘点和冻结库位</div><div class="rule-item"><i>✓</i>按拣选位下限、波次需求和通道拥堵综合排序</div></div>', event.target);
  else if (action === 'replenish-detail') openDrawer('补货任务详情', '<div class="detail-grid"><div class="detail-cell"><small>任务</small><b>RP260703-041</b></div><div class="detail-cell"><small>来源库位</small><b>A-03-11-04</b></div><div class="detail-cell"><small>目标库位</small><b>A-01-03-02</b></div><div class="detail-cell"><small>建议量</small><b>144 件</b></div></div><div class="timeline"><div class="timeline-item"><b>下限触发</b><small>当前库存低于安全库存</small></div><div class="timeline-item"><b>等待释放</b><small>释放后生成 PDA 任务</small></div></div>', event.target);
  else if (action === 'create-count') openDrawer('新建盘点计划', '<div class="form-grid"><div class="field"><label>盘点范围</label><select><option>拣选区 A</option><option>存储区 B</option><option>指定 SKU</option></select></div><div class="field"><label>盘点模式</label><select><option>盲盘</option><option>明盘</option><option>动碰盘点</option></select></div><div class="field"><label>冻结策略</label><select><option>冻结出库</option><option>仅提示风险</option></select></div><button class="btn primary" data-action="count-plan-save">创建并下发</button></div>', event.target);
  else if (action === 'count-plan-save') { const r=createCountPlan(state,{scope:'拣选区 A',mode:'盲盘',freezePolicy:'冻结出库'}); state=r.state; closeDrawer(); toast(r.message,r.ok,'盘点计划已下发'); render(); }
  else if (action === 'scan-rma') { const r=scanRma(state); state=r.state; toast(r.message,r.ok,'扫描成功'); render(); }
  else if (action === 'rule-detail') openDrawer('规则版本记录', '<div class="timeline"><div class="timeline-item"><b>v5 已发布</b><small>2026-07-02 · 林安 · 调整 B2C 截单优先级</small></div><div class="timeline-item"><b>v4 已归档</b><small>2026-06-28 · 系统保留历史任务引用</small></div><div class="timeline-item"><b>冲突检测通过</b><small>未发现范围和优先级冲突</small></div></div>', event.target);
  else if (action === 'reconcile') openDrawer('业务对账', '<div class="table-wrap"><table class="data-table"><thead><tr><th>链路</th><th>源系统</th><th>目标系统</th><th>差异</th><th>状态</th></tr></thead><tbody><tr><td>订单创建</td><td>OMS 2,486</td><td>WMS 2,486</td><td>0</td><td><span class="badge success"><i></i>一致</span></td></tr><tr><td>发运回传</td><td>WMS 1,936</td><td>TMS 1,936</td><td>0</td><td><span class="badge success"><i></i>一致</span></td></tr><tr><td>库存事务</td><td>WMS 318</td><td>ERP 318</td><td>0</td><td><span class="badge success"><i></i>一致</span></td></tr></tbody></table></div>', event.target);
  else if (action === 'integration-docs') openDrawer('接口目录', '<div class="rule-list"><div class="rule-item"><i>✓</i>OMS：order_created、order_cancelled、stock_query</div><div class="rule-item"><i>✓</i>TMS：shipment_confirmed、waybill_printed、carrier_handover</div><div class="rule-item"><i>✓</i>ERP：item_sync、inventory_transaction、merchant_sync</div><div class="rule-item"><i>✓</i>WCS：task_release、equipment_status、route_result</div></div><div class="weight-result">所有接口均展示幂等键、追踪 ID、重试策略和失败转人工规则。</div>', event.target);
  else if (action === 'trace-detail') openDrawer('接口链路追踪', '<div class="timeline"><div class="timeline-item"><b>OMS 创建订单</b><small>TRC-7D18C4 · 200 OK</small></div><div class="timeline-item"><b>WMS 完成库存分配</b><small>幂等键 ORD-SO20260703001</small></div><div class="timeline-item"><b>任务生成</b><small>波次 WV20260703-021</small></div></div>', event.target);
  else if (action === 'printer-test') { const r=createLitePrintJob(state,{templateId:'TPL-TEST',sourceId:'PRINTER-TEST'}); state=r.state; toast('测试页已进入打印队列，可在“打印队列”查看', true, '打印任务已创建'); render(); }
  else if (action === 'new-print') openDrawer('创建打印任务', '<form id="new-print-form" class="form-grid"><div class="field"><label for="print-template">模板</label><select id="print-template"><option value="TPL-SHP">出库单模板</option><option value="TPL-REC">入库单模板</option><option value="TPL-MOV">移库单模板</option><option value="TPL-CHK">盘库单模板</option></select></div><div class="field"><label for="print-source">来源单据</label><input id="print-source" value="DOC-260703-REC"></div><div class="field"><label for="print-copies">份数</label><input id="print-copies" type="number" min="1" max="10" value="1"></div><button class="btn primary" type="submit">生成打印任务</button></form>', event.target);
  else if (action === 'print-detail') openDrawer('打印任务详情', '<div class="detail-grid"><div class="detail-cell"><small>任务</small><b>PR260703-0916</b></div><div class="detail-cell"><small>模板</small><b>装箱单 v5</b></div><div class="detail-cell"><small>打印机</small><b>PT-06</b></div><div class="detail-cell"><small>状态</small><b>排队中</b></div></div><div class="timeline"><div class="timeline-item"><b>任务创建</b><small>复核工作站 PK-06</small></div><div class="timeline-item"><b>等待打印服务确认</b><small>可自动重试 3 次</small></div></div>', event.target);
  else if (action === 'lite-detail') openDrawer('业务配置详情', '<div class="detail-section"><h3>已接入操作</h3><div class="rule-list"><div class="rule-item"><i>✓</i>列表查询、筛选、分页和状态展示</div><div class="rule-item"><i>✓</i>新增、编辑、审核、启停、重试和打印任务</div><div class="rule-item"><i>✓</i>关键数据范围、库存事务和审计轨迹</div></div></div><div class="weight-result">当前操作写入本系统本地业务状态，用于页面联动和交互验证。</div>', event.target);
    ...
    const saved=saveLiteRecord(state,{module:'documents',values:{id:`DOC-260703-${state.lite.catalogs.documentTemplates.length + 1}`,name:'新增采购入库单',type:'入库',counterparty:'星云智能科技',quantity:48,amount:'¥32,640',status:'待审核'}});
    state=saved.state; closeDrawer(); toast('单据已暂存并进入待审核列表',saved.ok,'单据已创建'); render();
  }
  else if (action === 'lite-sync') { const r=syncMasterData(state); state=r.state; toast(r.message,r.ok,'同步完成'); render(); }
  else if (action === 'lite-new') {
    const module={ documents:'documents', transactions:'transactions', alerts:'warnings', printJobs:'printJobs' }[state.activeView] || ui.tabs.lite || 'warehouses';
  ...
  else if (action === 'lite-print-complete') { const r=completeLitePrintJob(state,event.target.closest('[data-lite-id]').dataset.liteId); state=r.state; toast(r.message,r.ok,r.ok?'打印完成':'打印被阻止'); render(); }
  else if (action === 'lite-transaction-detail') openDrawer('库存事务链路', '<div class="rule-list"><div class="rule-item"><i>✓</i>来源单据审核通过</div><div class="rule-item"><i>✓</i>库存事务入账</div><div class="rule-item"><i>✓</i>SKU/仓库预警当前值重算</div><div class="rule-item"><i>✓</i>审计记录保留</div></div>', event.target);
  ...
  else if (action === 'approve') { const r=resolveApproval(state,{id:event.target.closest('[data-approval-id]')?.dataset.approvalId || 'AP260703-018',decision:'approve'}); state=r.state; closeDrawer(); toast(r.message,r.ok,'审批完成'); render(); }
  else if (action === 'reject-approval') { const r=resolveApproval(state,{id:event.target.closest('[data-approval-id]')?.dataset.approvalId || 'AP260703-018',decision:'reject'}); state=r.state; closeDrawer(); toast(r.message,r.ok,'审批已驳回'); render(); }
  else if (action === 'retry-print') { const r=createLitePrintJob(state,{templateId:'TPL-SHP',sourceId:'PR260703-0917'}); state=r.state; toast(r.message,r.ok,'重试任务已创建'); render(); }
  else if (action === 'release-replenishment') { const r=releaseReplenishment(state); state=r.state; toast(r.message,r.ok,'补货任务已释放'); render(); }
  else if (action === 'rule-simulate') { const r=simulateRuleSample(state); state=r.state; toast(r.message,r.ok,'试算完成'); render(); }
  else if (action === 'sync-master') { const r=syncMasterData(state); state=r.state; toast(r.message,r.ok,'同步完成'); render(); }
  ...
  else if (action === 'save-location') { const r=saveLocation(state,{code:'A-01-03-04'}); state=r.state; closeDrawer(); toast(r.message,r.ok,'主数据已更新'); render(); }
  ...
  else if (action === 'warehouse-current') { closeDrawer(); toast('已保持当前运营仓：华东一号仓'); }
  ...
  else if (action === 'role-permission-detail') { const id=event.target.closest('[data-role-id]')?.dataset.roleId; const role=state.security.roles.find(item=>item.id===id); openDrawer('角色权限说明',`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px"><span><b>${role.name}</b><br><small class="mono">${role.id}</small></span>${badge(`${role.version} · ${role.status}`,role.status==='已发布'?'success':'muted')}</div><div class="detail-section" style="margin-top:18px"><h3>角色职责</h3><div class="weight-result">${role.duty||'自定义角色，职责待配置'}</div></div><div class="detail-grid"><div class="detail-cell"><small>角色类型</small><b>${role.type}</b></div><div class="detail-cell"><small>关联用户</small><b>${role.users} 人</b></div><div class="detail-cell"><small>权限项</small><b>${role.permissions} 项</b></div><div class="detail-cell"><small>数据范围</small><b>${role.scope}</b></div></div><div class="detail-section"><h3>可访问模块</h3><div class="rule-list">${(role.modules||'待配置').split('、').map(item=>`<div class="rule-item"><i>✓</i>${item}</div>`).join('')}</div></div><div class="detail-section"><h3>关键操作权限</h3><div class="rule-list">${(role.actions||'待配置').split('；').map(item=>`<div class="rule-item"><i>✓</i>${item}</div>`).join('')}</div></div><div class="detail-section"><h3>限制与职责分离</h3><div class="role-restriction">${role.restrictions||'遵循默认拒绝和最小权限原则'}</div></div>`,event.target); }
  ...
  else if (action === 'submit-audit-export') { const r=createExportJob(state,{type:'审计日志脱敏导出',scope:'当前筛选结果'}); state=r.state; closeDrawer(); toast(r.message,r.ok,'审计导出已生成'); render(); }
  ...
  else if (action === 'security-report-export') { const r=createExportJob(state,{type:'安全态势报告',scope:'脱敏安全指标'}); state=r.state; closeDrawer(); toast(r.message,r.ok,'安全报告已生成'); render(); }
  else if (action === 'retry') { const result=retryIntegration(state,'MSG-260703-0098'); state=result.state; toast(result.message,result.ok,result.ok?'接口恢复':'无法重试'); render(); }
  else if (action) toast(`操作 ${action} 已执行，结果已写入当前页面状态`);
...
document.addEventListener('submit', (event) => {
  ...
  if (event.target.id==='new-print-form') {
    event.preventDefault();
    const result=createLitePrintJob(state,{templateId:document.querySelector('#print-template').value,sourceId:document.querySelector('#print-source').value,copies:document.querySelector('#print-copies').value});
    state=result.state; closeDrawer(); toast(result.message,result.ok,'打印任务已创建'); render(); return;
  }
});
